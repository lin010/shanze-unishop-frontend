<template>
	<view class="sz-title" :class="[ui,align, tpl, bg, { line: inLine }, 'heading-' + depth]">
		<view class="anchor" :id="title" :style="{top:`-${sz_navBar}px`}"></view>
		<view class="action" :class="[hasLine ? 'hasLine' : '', hasDot ? 'hasDot' : '']">
			<div class="action-icon">
				<block v-if="isIcon">
					<view class="sz-TC-Main _icon-titles" v-if="depth < 3"></view>
					<view class="sz-TC-Main _icon-title" v-if="depth == 3"></view>
					<view class="sz-TC-Main _icon-title icon-sm" v-if="depth > 3"></view>
				</block>
				<slot name="icon"></slot>
			</div>
					...
					.action-line {
						...
						view {
							...
							&::after {
								...
								width: 100%;
								height: 100%;
								background: linear-gradient(to left, var(--sz-BG), rgba(255, 255, 255, 0));
								display: block;
								position: absolute;
							}
						}
